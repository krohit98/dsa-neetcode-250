class Solution {
    ...
    getOrder(tasks) {
        ...
        let processHeap = new PriorityQueue((taskA, taskB) => {
            return taskA.processTime == taskB.processTime ? taskA.index - taskB.index : taskA.processTime - taskB.processTime;
        })
        ...
        while(enqueueHeap.size() || processHeap.size()) {
            ...
            if(enqueueHeap.size() && time < enqueueHeap.front().enqueueTime) {
                time = enqueueHeap.front().enqueueTime
            }

            while(enqueueHeap.size() && enqueueHeap.front().enqueueTime <= time) {
