...
public:
    bool isValidSudoku(vector<vector<char>>& board) {
        ...
        for(auto x:digits) cout<<x.first<<":"<<x.second<<endl;
        cout<<"*****************"<<endl;

        // check duplicates in rows
        for(int i=0;i<9;i++){
            for(int j=0;j<9;j++){
                if(digits[board[i][j]] == true){
                    return false;
                }
                if(board[i][j]!='.') digits[board[i][j]] = true;
                cout<<board[i][j]<<":"<<digits[board[i][j]]<<endl;
            }
            cout<<"2*****************2"<<endl;
            // reset map
            for(int i=1;i<=9;i++) digits[i+'0'] = false;
            ...
            for(int h=0;h<=6;h+=3){
                for(int i=0+k;i<3+k;i++){
                    for(int j=0+h;j<3+h;j++){
                        if(digits[board[i][j]] == true) return false;
                        if(board[i][j]!='.') digits[board[i][j]] = true;
                    }
                }
            // reset map
